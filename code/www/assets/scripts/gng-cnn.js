...
var Region = {
  ...
  regionSuffixes : [ "-input", "-inverse", "-output", "-layer-conv-error-0", "-layer-conv-error-1", "-layer-conv-best-0", "-layer-conv-best-1", "-layer-pool-error-0", "-layer-pool-error-1", "-layer-pool-best-0", "-layer-pool-best-1" ],
  regionSuffixIdx : 0,
  dataMap : {
  },
...
//    $( "#mouse-right" ).html( status );
  ...
  repaint : function() {
    Region.repaintInput( "#left-canvas" );
    Region.repaintConv( "#centre-canvas" );
    Region.repaintPool( "#right-canvas" );
  },

  getDataSize2d : function( dataSize3d ) {
    var dataSize2d = {
      w: dataSize3d.w * dataSize3d.d,
      h: dataSize3d.h
    };
  ...
  getDataSize2dDepth : function( dataSize3d ) {
    var dataSize2d = {
      w: dataSize3d.d,
      h: dataSize3d.w * dataSize3d.h
    };
 
    return dataSize2d;   
  },

  repaintInput : function( canvasSelector ) {
    var dataInput = Region.findData( "-input" );
    if( !dataInput ) {
      return; // can't paint
    }
    var dataInverse = Region.findData( "-inverse" );
    if( !dataInverse ) {
      return; // can't paint
    }

    var dataSize3d = Framework.getDataSize3d( dataInput );
    var dataSize2d = Region.getDataSize2d( dataSize3d );

    var ctx = Region.resizeCanvas( canvasSelector, dataSize2d, 1, 2 );

    var x0 = 0;
    var y0 = 0;

    Region.fillDataRgbBinary( ctx, x0, y0, dataSize2d.w, dataSize2d.h, dataInput, null, null );//, null );

    y0 += ( dataSize2d.h * Region.pixelsPerBit + Region.pixelsPerGap );

    Region.fillDataRgba( ctx, x0, y0, dataSize2d.w, dataSize2d.h, dataInverse, 0,255,0, 1.0 );//, null );

    Region.strokeElements( ctx, x0, y0, dataSize2d.w, dataSize2d.h, Region.selectedCells, "#00ffff" );
  },
...
//  regionSuffixes : [ "-input", "-inverse", "-output", "-layer-conv-error-0", "-layer-conv-error-1", "-layer-conv-best-0", "-layer-conv-best-1", "-layer-pool-error-0", "-layer-pool-error-1", "-layer-pool-best-0", "-layer-pool-best-1" ],
  ...
  repaintConv : function( canvasSelector ) {

    var layer = Region.getLayer();
    var dataBest = Region.findData( "-layer-conv-best-" +layer );
    if( !dataBest ) {
      return; // can't paint
    }
    var dataError = Region.findData( "-layer-conv-error-" +layer );
    if( !dataError ) {
      return; // can't paint
    }

    var dataSize3d = Framework.getDataSize3d( dataBest );
    var dataSize2d = Region.getDataSize2dDepth( dataSize3d );

    var ctx = Region.resizeCanvas( canvasSelector, dataSize2d, 1, 1 );

    var x0 = 0;
    var y0 = 0;

    //Region.fillDataRgbBinary( ctx, x0, y0, dataSize2d.w, dataSize2d.h, dataError, dataBest, null );//, null );
    DataCanvas.fillElementsUnitRgb( ctx, x0, y0, dataSize2d.w, dataSize2d.h, dataError, dataBest, null );
    Region.strokeElements( ctx, x0, y0, dataSize2d.w, dataSize2d.h, Region.selectedCells, "#00ffff" );
  },

  repaintPool : function( canvasSelector ) {

    var layer = Region.getLayer();
    var dataBest = Region.findData( "-layer-pool-best-" +layer );
    if( !dataBest ) {
      return; // can't paint
    }
    var dataError = Region.findData( "-layer-pool-error-" +layer );
    if( !dataError ) {
      return; // can't paint
    }
...
/*        if( selectedElements ) {
  ...
  resizeCanvas : function( canvasSelector, dataSize2d, repeatX, repeatY ) {
    if( ( dataSize2d.w == 0 ) || ( dataSize2d.h == 0 ) ) {
      return null;
    }
    ...
    c.width  = ( dataSize2d.w * Region.pixelsPerBit ) * repeatX + Region.pixelsPerGap * (repeatX -1);
    c.height = ( dataSize2d.h * Region.pixelsPerBit ) * repeatY + Region.pixelsPerGap * (repeatY -1);
