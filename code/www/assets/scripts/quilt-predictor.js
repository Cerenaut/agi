...
var Region = {
  ...
  regionSuffixes : [ "-input-c", "-prediction-old", "-prediction-new", "-prediction-new-unit" ],
...
//    Region.selectCells( "-classifier-spikes-old" );
  ...
  selectThreshold : function() {
    var data1 = Region.findData( "-prediction-new-unit" );
    if( !data1 ) {
      return; // can't paint
    }
...
//    $( "#mouse-right" ).html( status );
  ...
  onMouseClickCentre : function( e, mx, my ) {
    ...
    var data = Region.findData( "-input-c" );
    if( !data ) {
      return; // can't paint
    }
  ...
  // left: input / pred old
  // middle: input / pred new
  // right: pred unit

  repaintLeft : function() {
    var input = Region.findData( "-input-c" );
    if( !input ) {
      return; // can't paint
    }

    var prediction = Region.findData( "-prediction-old" );
    if( !prediction ) {
      return; // can't paint
    }

    var panels = 1;
    var canvasDataSize = Region.resizeCanvas( "#left-canvas", input, 1, panels );
    if( !canvasDataSize ) {
      return; // can't paint
    }
    ...
    DataCanvas.fillElementsUnitRgb( canvasDataSize.ctx, x0, y0, canvasDataSize.w, canvasDataSize.h, prediction, input, null );
  ...
  repaintMiddle : function() {
    ...
    var prediction = Region.findData( "-prediction-new" );
    if( !prediction ) {
      return; // can't paint
    }

    var panels = 1;
    var canvasDataSize = Region.resizeCanvas( "#centre-canvas", input, 1, panels );
    if( !canvasDataSize ) {
      return; // can't paint
    }
  ...
  repaintRight : function() {
    ...
    var prediction = Region.findData( "-prediction-new-unit" );
    if( !prediction ) {
      return; // can't paint
    }

    var panels = 1;
    var canvasDataSize = Region.resizeCanvas( "#right-canvas", prediction, 1, panels );
    if( !canvasDataSize ) {
      return; // can't paint
    }
