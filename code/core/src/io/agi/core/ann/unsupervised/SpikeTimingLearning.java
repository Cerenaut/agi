 ...
 */
public class SpikeTimingLearning {
...
//                wCount += 1;
    ...
    public void train( Data inputSpikes, Data outputSpikes ) {
        ...
        int inputs = inputSpikes.getSize();
        int outputs = outputSpikes.getSize();
        ...
        for( int i = 0; i < inputs; ++i ) { // pre
            ...
            float preSpike = inputSpikes._values[ i ];

            for( int j = 0; j < outputs; ++j ) { // post

                int offset = i * outputs + j; // the weight from state s1, to state s2.

//                float postOld = outputOld._values[ j ];
                float postNew = outputSpikes._values[ j ];

                float delta = 0f;

//                if( ( postNew == 0f ) && ( postNew == 0f ) ) {
                if( postNew == 0f ) {
                    // no new postsynaptic spike. Is there a pre-synaptic spike and an OLD trace of a post spike?
                    if( preSpike == 1f ) {
                        // The postsynaptic trace is zero when enough time has passed - the pre-spike is much later than the post spike.
                        float postTrace = _inputOutputTraces._values[ j ];
                        delta -= _learningRate * postTrace;
                    }
                }
                else { // new post. syn. spike has happened
                    // The input (presynaptic) trace is older than the post-synaptic trace now.
                    // If the preTrace is 0 (doesn't exist), then no change anyway.
                    // So this is + (predictive)
                    float preTrace = _inputOutputTraces._values[ offset ];
                    delta += _learningRate * preTrace; // increase it by whatever the pre-synaptic trace value is. if the pre spike occurs immediately before the

                    // now cancel the preTrace for this output.
                    // We don't want to use it more than once.
                    _inputOutputTraces._values[ offset ] = 0f;
                }

                // if nothing is changing, no more to do
                if( delta == 0f ) {
